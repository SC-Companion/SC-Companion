...
// User schemas - SC Companion user with RSI data included
export const CreateUserSchema = z.object({
  ...
  // RSI data (optional at registration, will be linked later)
  rsi_handle: z.string().min(1).max(50).optional(),
  // rsi_geid will be populated automatically from game logs via desktop app
});
...
export const UpdateUserSchema = z.object({
  ...
  // RSI data
  rsi_handle: z.string().min(1).max(50).optional(),
  // rsi_geid is updated automatically via desktop app, not manually
});
...
export const BanUserSchema = z.object({
  ...
  duration: z.number().positive().optional(), // Duration in hours, undefined = permanent
});
